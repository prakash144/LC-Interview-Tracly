...
const PageHeader = ({ title, description, eyebrow, actions }: PageHeaderProps) => (
  <section className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8">
    <div className="min-w-0">
      {eyebrow && (
        <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-success flex items-center gap-2">
          <span className="size-1.5 rounded-full bg-success/60" />
          {eyebrow}
        </div>
      )}
      <h1 className="text-2xl font-semibold text-foreground sm:text-3xl tracking-tight">{title}</h1>
      {description && (
